...
def RelDielConst_SalineWater(t, f, S):
    """
    ...
    sig35 = A[0] + A[1] * t + A[2] * t**2 + A[3] * t**3 + A[4] * t**4

    A = [37.5109, 5.45216, 0.014409, 1004.75, 182.283]
    P = S * ((A[0] + A[1]*S + A[2]*S**2) / (A[3] + A[4]*S + S**2))

    A = [6.9431, 3.2841, -0.099486, 84.85, 69.024]
    alpha0 = (A[0] + A[1]*S + A[2]*S**2) / (A[3] + A[4]*S + S**2)

    A = [49.843, -0.2276, 0.00198]
    alpha1 = A[0] + A[1]*S + A[2]*S**2
    ...
    epsS = 87.85306 * np.exp(-0.00456992 * t - a[0] * S - a[1] * S**2 - a[2] * S * t)
    epsOne = a[3] * np.exp(-a[4] * t - a[5] * S - a[6] * S * t)
    ...
    eps = (((epsS-epsOne)/(1 - 1j * 2 * pi * f * tau1)) 
           + ((epsOne - epsInf) / (1 - 1j * 2 * pi * f * tau2)) 
           + (epsInf) + 1j * ((17.9751 * sigma)/f))
