...
# Original FBEM code : ice_backscatter.m
...
def ice_backscatter(Lambda, sigma_si, l_si, T_si, S_si, h_s, beta_c, epsr_ds):
    ...
    if h_s > 0:
        ...
        eta_2 = eta_0/sqrt(np.real(eps_si))
        theta_2 = np.arcsin(sin(theta) * (sqrt(epsr_a/epsr_ds))) # transmission angle
    else:
        eta_0 = 376.73031346177 # intrinsic impedance of free space, ohms
        eta_1 = eta_0/sqrt(epsr_a)
        eta_2 = eta_0/sqrt(np.real(eps_si))
        theta_2 = np.arcsin(sin(theta)*(sqrt(epsr_a/np.real(eps_si)))) #transmission angle
    ...
    sigma_0_HH_coh = 4 * ((gamma_H)/beta_c**2) * exp(-4 * k0**2 * sigma_si**2) * exp(-4 * theta**2/beta_c**2) # equation 6 of Fung and Eom 1983
    sigma_0_VV_coh = 4 * ((gamma_V)/beta_c**2) * exp(-4 * k0**2 * sigma_si**2) * exp(-4 * theta**2/beta_c**2)
    ...
    sigma_0_VV_si_surf[np.isinf(sigma_0_VV_si_surf)] = np.nan


    # Build spline interpolants (assumption that scattering is polarization-independent)
    # dB
    sigma_0_ice_surf = CubicSpline(theta,(sigma_0_HH_si_surf + sigma_0_VV_si_surf)/2);


    return theta, sigma_0_ice_surf, tau_ice